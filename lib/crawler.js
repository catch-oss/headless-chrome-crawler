...
class Crawler {
  ...
  async _waitFor() {
    ...
    const { selectorOrFunctionOrTimeout, options, args = [] } = this._options.waitFor;
    if (typeof selectorOrFunctionOrTimeout === 'number') {
      await delay(selectorOrFunctionOrTimeout);
      return;
    }
    if (typeof selectorOrFunctionOrTimeout === 'string') {
      const target = selectorOrFunctionOrTimeout.trim();
      const isFunctionLike = target.startsWith('function') || target.startsWith('(');
      if (isFunctionLike) {
        await this._page.waitForFunction(target, options, ...args);
      } else if (target.startsWith('//')) {
        await this._page.waitForFunction(
          xpath => Boolean(document.evaluate(xpath, document, null, 9, null).singleNodeValue),
          options,
          target,
        );
      } else {
        await this._page.waitForSelector(target, options);
      }
      return;
    }
    await this._page.waitForFunction(selectorOrFunctionOrTimeout, options, ...args);
  }
