...
const robotsParserModule = require('robots-parser');
const robotsParser = /** @type {any} */ (robotsParserModule.default || robotsParserModule);
...
class HCCrawler extends EventEmitter {
  ...
  */
  static async launch(options) {
    ...
    const disableUpdater = process.env.PUPPETEER_SKIP_CHROMIUM_DOWNLOAD || process.env.PUPPETEER_EXECUTABLE_PATH;
    if (launchOptions.args || disableUpdater) {
      const args = launchOptions.args ? [].concat(launchOptions.args) : [];
      const ensureFlag = flag => {
        if (!args.includes(flag)) args.push(flag);
      };
      if (disableUpdater) {
        ensureFlag('--disable-component-update');
        ensureFlag('--disable-background-networking');
      }
  ...
  constructor(browser, options) {
    ...
    this._ensureBrowserConnectionShim();
  ...
  /**
   * Keep compatibility for consumers that rely on _browser._connection._closed.
   * @private
   */
  _ensureBrowserConnectionShim() {
    const { _browser: browser } = this;
    // Probe for the property, not a method: puppeteer 25 replaced
    // browser.isConnected() with browser.connected, and 24 has both.
    if (!browser || typeof browser.connected !== 'boolean') return;

    const anyBrowser = /** @type {any} */ (browser);
    const { _connection: connection } = anyBrowser;
    if (connection && Object.prototype.hasOwnProperty.call(connection, '_closed')) return;

    let disconnected = false;
    browser.once('disconnected', () => { disconnected = true; });

    const target = connection || {};
    Object.defineProperty(target, '_closed', {
      configurable: true,
      enumerable: false,
      // Track disconnection ourselves rather than asking the browser.
      //
      // This getter backs `_closed`, and puppeteer's own `connected` getter is
      // `!this.#connection._closed` - so reading browser.connected (or
      // isConnected(), which delegates to it) from in here re-enters this
      // getter and blows the stack. Launch then crawl reproduced
      // "RangeError: Maximum call stack size exceeded" three times out of
      // three before this changed.
      get: () => disconnected,
    });

    if (connection) return;

    Object.defineProperty(anyBrowser, '_connection', {
      configurable: true,
      enumerable: false,
      writable: true,
      value: target,
   ...
   */
  userAgent() {
    if (!this._browser.connected) return; // catch error after scan
  ...
  async _newCrawler(options, depth, previousUrl) {
    try {
      ...
      if (!this._browser.connected) return; // catch error after scan
      debugConsole(`Error while _newCrawler: ${e instanceof Error ? e.stack || e.message : String(e)}`);
    }
  }
  ...
  async _crawl(crawler) {
    ...
    return this._customCrawl(crawler.page(), crawl, crawler);
  }
